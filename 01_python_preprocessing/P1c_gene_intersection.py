...
def main(args=None):
    # --- CLI parameter resolution (SPEC v2 §2; P1c refactor) ---
    # P1c is fan-in (no --sample); --registry/--indir/--outdir override
    # the defaults.  No-arg invocation preserves pre-refactor behaviour.
    global REGISTRY_PATH, P1_RESULT_DIR, OUT_DIR
    if args is not None:
        if args.registry:
            REGISTRY_PATH = args.registry
        if args.indir:
            P1_RESULT_DIR = args.indir
        if args.outdir:
            OUT_DIR = args.outdir
    ...
    # --- resolve h5 paths: explicit --manifest or nested --indir ---
    manifest = {}
    if args is not None and args.manifest:
        with open(args.manifest, "r") as f:
            for line in f:
                ...
                parts = line.split("\t")
                if len(parts) != 2:
                    print(f"  manifest line malformed (need TAB-separated): {line}")
                    sys.exit(1)
    ...
    for sample_name, info in registry.items():
        h5_path = manifest.get(sample_name,
               os.path.join(P1_RESULT_DIR, sample_name, "filtered_matrix.h5"))
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(
        description="P1c: cross-dataset gene intersection analysis (fan-in)")
    ap.add_argument("--registry", default=None,
                    help="sample_registry.json path (default: "
                         "script-dir/sample_registry.json)")
    ap.add_argument("--manifest", default=None,
                    help="explicit h5 file manifest, one per line "
                         "(format: Dataset/Subname<TAB>path); overrides "
                         "--indir for Nextflow work-dir consumption)")
    ap.add_argument("--indir", default=None,
                    help="P1 results root (default: "
                         "$RESULTS_DIR/P1_Results)")
    ap.add_argument("--outdir", default=None,
                    help="output directory (default: "
                         "$INDIR/Gene_Intersection)")
    args = ap.parse_args()
    main(args)
