...
def main(args=None):
    # --- CLI parameter resolution (SPEC v2 §2; P1b refactor) ---
    # Defaults preserve the pre-refactor behaviour EXACTLY when no
    # --sample is given (hard constraint 3: byte-equivalent full mode).
    global REGISTRY_PATH, RESULT_DIR
    if args is not None:
        if args.registry:
            REGISTRY_PATH = args.registry
        if args.outdir:
            RESULT_DIR = args.outdir
    sample_filter = getattr(args, "sample", None) if args else None
    single_sample_mode = sample_filter is not None
    ...
    if single_sample_mode:
        if sample_filter not in registry:
            print(f"  ✗ --sample '{sample_filter}' not in registry\n")
            sys.exit(1)
        registry = {sample_filter: registry[sample_filter]}
    ...
    for i, (sample_name, info) in enumerate(registry.items(), 1):
        ...
        sample_out = os.path.join(RESULT_DIR, sample_name) \
            if not single_sample_mode else RESULT_DIR
    ...
    # 阶段 2: 汇总 QC
    # ===========================================================
    # single-sample mode: write a shard (p1_qc.csv) for fan-in merge;
    # full mode: write ALL_SAMPLES_P1_QC.csv (byte-equivalent to pre-refactor)
    if single_sample_mode:
        print(f"\n  单样本模式: 写 p1_qc.csv 分片")
        qc_df = pd.DataFrame(all_qc)
        qc_path = os.path.join(RESULT_DIR, "p1_qc.csv")
        qc_df.to_csv(qc_path, index=False)
        print(f"  QC 分片已保存: {qc_path}")
    else:
        print(f"\n{'=' * 65}")
        print("  阶段 2: 汇总 QC")
        print(f"{'=' * 65}")
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="P1b: load/filter Xenium data")
    ap.add_argument("--sample", default=None,
                    help="process a single sample (Dataset/Subname); "
                         "omit for full-batch mode")
    ap.add_argument("--registry", default=None,
                    help="sample_registry.json path (default: "
                         "script-dir/sample_registry.json)")
    ap.add_argument("--indir", default=None,
                    help="input root (unused — input is per registry path)")
