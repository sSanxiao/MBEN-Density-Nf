...
if (!is.null(.args$indir) && !isTRUE(.args$indir)) R3_DIR <- .args$indir
...
for (i in seq_along(sample_names)) {
  ...
  if (single_sample_mode) {
    r3_path <- file.path(R3_DIR, "density_gene_correlations.csv")
  } else {
...
if (single_sample_mode) {
  summary_path <- file.path(OUTPUT_DIR, "r4_summary.csv")
  fwrite(summary_df, summary_path)
  cat("r4_summary.csv shard saved:", summary_path, "\n\n")
} else {
