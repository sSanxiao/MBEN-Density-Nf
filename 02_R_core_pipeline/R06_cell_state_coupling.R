...
# ============================================================================
# CLI parameter resolution (S5b refactor; SPEC v2 §2)
# ============================================================================
...
if (single_sample_mode) {
  if (is.null(.args$outdir) || isTRUE(.args$outdir))
    stop("--outdir is required in --sample mode")
...
for (i in seq_along(sample_names)) {
  ...
  if (single_sample_mode) {
    rds_path <- file.path(R2_DIR, paste0(sample_subname, "_seurat_R2.rds"))
    r4_path <- file.path(R4_DIR, "filtered_density_genes.csv")
  } else {
...
if (single_sample_mode) {
  fwrite(summary_df, file.path(OUTPUT_DIR, "r6_summary.csv"))
  cat("r6_summary.csv shard saved\n")
} else {
