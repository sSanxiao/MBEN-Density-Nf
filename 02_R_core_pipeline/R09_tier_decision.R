...
if (!is.null(.args$indir) && !isTRUE(.args$indir)) {
  R2_DIR <- .args$indir; R3_DIR <- .args$indir; R4_DIR <- .args$indir
}
if (!is.null(.args$outdir) && !isTRUE(.args$outdir)) OUT_DIR <- .args$outdir

# --- --manifest: explicit keyed file list (Q4 strict validation) ---
# Required types for R09: density_gene_correlations.csv (R3),
#   filtered_density_genes.csv (R4), <Subname>_seurat_R2.rds (R2).
manifest_map <- list()
...
if (has_manifest) {
  ...
  for (ln in lines) {
    ...
    is_r3 <- bname == "density_gene_correlations.csv"
    is_r4 <- bname == "filtered_density_genes.csv"
    is_r2 <- grepl("_seurat_R2\\.rds$", bname)
    if (!(is_r3 || is_r4 || is_r2))
  ...
  n_r3 <- sum(sapply(manifest_map, function(m) any(grepl("density_gene_correlations", names(m)))))
  n_r4 <- sum(sapply(manifest_map, function(m) any(grepl("filtered_density_genes", names(m)))))
  n_r2 <- sum(sapply(manifest_map, function(m) any(grepl("_seurat_R2", names(m)))))
  ...
  cat("  _seurat_R2.rds:               ", n_r2, "entries\n\n")
}
...
for (mb_s in mb_samples) {
  ...
  if (has_manifest && mb_s %in% names(manifest_map)) {
    r4_path <- manifest_map[[mb_s]][["filtered_density_genes.csv"]]
  } else {
...
for (i in seq_along(sample_names)) {
  ...
  if (has_manifest && sname %in% names(manifest_map)) {
    rds_key <- grep("_seurat_R2", names(manifest_map[[sname]]), value = TRUE)[1]
    rds_path <- if (!is.na(rds_key)) manifest_map[[sname]][[rds_key]]
                else file.path(R2_DIR, ps$dataset, ps$subname,
