...
# ============================================================================
# CLI parameter resolution (S5b refactor; SPEC v2 §2 + Q4)
# ============================================================================

.script_dir <- tryCatch({
  pp <- sub("--file=", "", grep("^--file=", commandArgs(FALSE), value = TRUE))
  if (length(pp) > 0) normalizePath(dirname(pp[1]), winslash = "/") else "."
}, error = function(e) ".")
...
if (!is.null(.args$registry) && !isTRUE(.args$registry)) REGISTRY_PATH <- .args$registry
if (!is.null(.args$indir) && !isTRUE(.args$indir)) {
  R3_DIR <- .args$indir; R4_DIR <- .args$indir; R6_DIR <- .args$indir
}
if (!is.null(.args$outdir) && !isTRUE(.args$outdir)) OUTPUT_DIR <- .args$outdir

CONSISTENCY_THRESHOLD <- 0.5

# --- --manifest: explicit keyed file list (Q4 strict validation) ---
# Format: <Dataset/Subname>\t<path>, classified by basename.
# Required types for R07: density_gene_correlations.csv (R3),
#   filtered_density_genes.csv (R4), cell_state_coupling.csv (R6, optional).
manifest_map <- list()  # list of named paths, keyed by basename
has_manifest <- !is.null(.args$manifest) && !isTRUE(.args$manifest)
if (has_manifest) {
  lines <- readLines(.args$manifest)
  for (ln in lines) {
    ln <- trimws(ln)
    if (ln == "") next
    parts <- strsplit(ln, "\t")[[1]]
    if (length(parts) != 2L)
      stop("manifest line malformed (need TAB-separated): ", ln)
    sname <- parts[1]; fpath <- parts[2]
    bname <- basename(fpath)
    if (!(bname %in% c("density_gene_correlations.csv",
                       "filtered_density_genes.csv",
                       "cell_state_coupling.csv")))
      stop("manifest basename cannot be classified: ", bname,
           " (for sample ", sname, ")")
    if (is.null(manifest_map[[sname]])) manifest_map[[sname]] <- list()
    manifest_map[[sname]][[bname]] <- fpath
  }
  # Q4(b): required types must have at least one entry
  n_r3 <- sum(sapply(manifest_map, function(m) "density_gene_correlations.csv" %in% names(m)))
  n_r4 <- sum(sapply(manifest_map, function(m) "filtered_density_genes.csv" %in% names(m)))
  if (n_r3 == 0) stop("manifest has 0 entries for density_gene_correlations.csv")
  if (n_r4 == 0) stop("manifest has 0 entries for filtered_density_genes.csv")
  # Q4(c): print coverage
  cat("Manifest coverage:\n")
  cat("  density_gene_correlations.csv:", n_r3, "entries\n")
  cat("  filtered_density_genes.csv:   ", n_r4, "entries\n")
  n_r6 <- sum(sapply(manifest_map, function(m) "cell_state_coupling.csv" %in% names(m)))
  cat("  cell_state_coupling.csv (optional):", n_r6, "entries\n\n")
}
...
for (i in seq_along(sample_names)) {
  ...
  # --- resolve paths: manifest (if provided) or registry-driven ---
  if (has_manifest && sname %in% names(manifest_map)) {
    r3_path <- manifest_map[[sname]][["density_gene_correlations.csv"]]
  } else {
  ...
  # --- 读取 R4 结果 ---
  if (has_manifest && sname %in% names(manifest_map)) {
    r4_path <- manifest_map[[sname]][["filtered_density_genes.csv"]]
  } else {
  ...
  # --- 读取 R6 结果 ---
  if (has_manifest && sname %in% names(manifest_map) && "cell_state_coupling.csv" %in% names(manifest_map[[sname]])) {
    r6_path <- manifest_map[[sname]][["cell_state_coupling.csv"]]
  } else {
