...
# ============================================================================
# CLI parameter resolution (S5a refactor; SPEC v2 §2)
# ============================================================================

# Source the minimal --key value parser (SPEC v2 §2.1)
.script_dir <- tryCatch({
...
if (file.exists(.args_path)) source(.args_path) else stop("config/args.R not found at: ", .args_path)
set.seed(42)
...
# CLI overrides (Q2: --registry default preserves existing $DATA_DIR behaviour)
...
# --sample mode: flat output to --outdir (required for single-sample)
single_sample_mode <- !is.null(.args$sample) && !isTRUE(.args$sample)
if (single_sample_mode) {
  if (is.null(.args$outdir) || isTRUE(.args$outdir))
    stop("--outdir is required in --sample mode (flat output, no shared root)")
  OUTPUT_DIR <- .args$outdir
}

sample_filter <- if (single_sample_mode) .args$sample else NULL
...
# single-sample mode: restrict to the requested sample
if (single_sample_mode) {
  if (!(sample_filter %in% names(registry)))
    stop("--sample '", sample_filter, "' not in registry")
  registry <- registry[sample_filter]
}
...
for (i in seq_along(sample_names)) {
  ...
  # input paths: flat (--sample mode, Nextflow work dir) or nested (full mode)
  if (single_sample_mode) {
    h5_path      <- file.path(P1_DIR, "filtered_matrix.h5")
    meta_path    <- file.path(P1_DIR, "cell_metadata.csv")
    density_path <- file.path(P2_DIR, "cell_density.csv")
  } else {
  ...
  # output: flat to OUTPUT_DIR in single-sample mode, nested in full mode
  if (single_sample_mode) {
    out_dir <- OUTPUT_DIR
  } else {
...
# single-sample mode: write r1_qc.csv shard (not ALL_SAMPLES_R1_QC.csv)
if (single_sample_mode) {
  qc_path <- file.path(OUTPUT_DIR, "r1_qc.csv")
  fwrite(qc_df, qc_path)
  cat("r1_qc.csv shard saved:", qc_path, "\n\n")
} else {
