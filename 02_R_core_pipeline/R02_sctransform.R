...
if (!is.null(.args$indir) && !isTRUE(.args$indir)) R1_DIR <- .args$indir
...
for (i in seq_along(sample_names)) {
  ...
  if (single_sample_mode) {
    rds_path <- file.path(R1_DIR, paste0(sample_subname, "_seurat.rds"))
  } else {
...
if (single_sample_mode) {
  qc_path <- file.path(OUTPUT_DIR, "r2_qc.csv")
  fwrite(qc_df, qc_path)
  cat("r2_qc.csv shard saved:", qc_path, "\n\n")
} else {
